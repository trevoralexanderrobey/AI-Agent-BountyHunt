...
# Deterministic PATH for Antigravity-launched subprocesses.
export PATH="/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"

readonly LOG_FILE="${HOME}/Library/Logs/antigravity-openclaw-bootstrap.log"
readonly HEALTH_TIMEOUT_SECONDS="${ANTIGRAVITY_OPENCLAW_HEALTH_TIMEOUT_SECONDS:-20}"
...
main() {
  ...
  if ! bootstrap_if_needed; then
    # Keep launching MCP so Antigravity can still connect and report tool-level errors.
